...
mod tests {
    ...
    use mc_transaction_core::{tx::TxOut, MemoPayload};
    ...
    fn test_memo_authentication() {
        ...
        println!(
            ...
            hex::encode(mc_util_serial::encode(&alice_addr.clone()))
        );
        let address_hash = ShortAddressHash::from(&alice_addr);
        println!(
            "short address hash : {}",
            hex::encode(<[u8; 16]>::from(address_hash))
        );
        println!(
            "recipient private key: {}",
            hex::encode(mc_util_serial::encode(
                &bob.clone().default_subaddress_view_private()
            ))
        );
        println!(
            "tx out public key: {}",
            hex::encode(mc_util_serial::encode(&tx_public_key.clone()))
        );
        ...
        let memo_data: [u8; 44] = <[u8; 44]>::from(memo2);
        println!("{}", hex::encode(memo_data));
        assert_eq!(
    ...
    fn test_destination_memo() {
        ...
        println!(
            "Sender Account Key: {}",
            hex::encode(mc_util_serial::encode(&alice.clone()))
        );
        println!(
            "Recipient Account Key: {}",
            hex::encode(mc_util_serial::encode(&bob.clone()))
        );
        //println!("Sender view private key: {}",
        // hex::encode(mc_util_serial::encode(&bob.view_private_key().clone())));
        // println!("Sender spend private key: {}",
        // hex::encode(mc_util_serial::encode(&bob.spend_private_key().clone())));
        // println!("Destination public key: {}",
        // hex::encode(mc_util_serial::encode(&alice_addr.clone())));

        let mut memo =
            DestinationMemo::new(ShortAddressHash::from(&alice_addr), 12u64, 13u64).unwrap();
        let memo_data: [u8; 44] = <[u8; 44]>::from(memo.clone());
        // println!("Memo Data: {}", hex::encode(memo_data));
        let memo_val = MemoPayload::new([2u8; 2], memo_data);
        let bob_addr = bob.change_subaddress();
        let tx_private_key = RistrettoPrivate::from_random(&mut rng);
        let tx_out: mc_transaction_core::tx::TxOut = TxOut::new_with_memo(
            13u64,
            &bob_addr,
            &tx_private_key,
            Default::default(),
        ...
        println!(
            "txout.target_key: {}",
            hex::encode(mc_util_serial::encode(&tx_out.target_key.clone()))
        );
        println!(
            "txout.public_key: {}",
            hex::encode(mc_util_serial::encode(&tx_out.public_key.clone()))
        );

        let external_tx_out: mc_api::external::TxOut =
            mc_api::external::TxOut::from(&tx_out.clone());
        println!(
            "new tx Out: {}",
            hex::encode(
                external_tx_out
